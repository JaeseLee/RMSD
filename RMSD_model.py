...
Y_trn = np.load(os.path.join(data_dir, 'trn_Y_R_Nir_NDVI_raw.npy'))
X_tst = np.load(os.path.join(data_dir, 'tst_X_VV_VH_RVI_raw.npy'))
Y_tst = np.load(os.path.join(data_dir, 'tst_Y_R_Nir_NDVI_raw.npy'))
...
def plot_validation_grid(X, Y_true, Y_pred, mask, save_path, epoch, sample_idx=0):
    X = X.detach().cpu().numpy()
    Y_true = Y_true.detach().cpu().numpy()
    Y_pred = Y_pred.detach().cpu().numpy()
    mask = mask.detach().cpu().numpy().astype(bool)

    sample_idx = min(sample_idx, X.shape[0] - 1)
    X_sample = X[sample_idx]
    Y_true_sample = Y_true[sample_idx].copy()
    Y_pred_sample = Y_pred[sample_idx].copy()
    mask_sample = mask[sample_idx]

    Y_true_sample[~mask_sample] = np.nan
    Y_pred_sample[~mask_sample] = np.nan

    rows = [
        ("X", X_sample, ["VV", "VH", "RVI"]),
        ("Y_pred", Y_pred_sample, ["R", "NIR", "NDVI"]),
        ("Y_true", Y_true_sample, ["R", "NIR", "NDVI"]),
    ]

    fig, axes = plt.subplots(3, 3, figsize=(11, 9), dpi=180, constrained_layout=True)
    ...
    fig.suptitle(f"Validation sample - epoch {epoch + 1:03d}", fontsize=12)
...
# 3-sigma masking
X_trn_3sig = mask_outside_nsigma(X_trn, X_trn_mean, X_trn_std, nsigma=3)
X_tst_3sig = mask_outside_nsigma(X_tst, X_trn_mean, X_trn_std, nsigma=3)

Y_trn_3sig = mask_outside_nsigma(Y_trn, Y_trn_mean, Y_trn_std, nsigma=3)
Y_tst_3sig = mask_outside_nsigma(Y_tst, Y_trn_mean, Y_trn_std, nsigma=3)
...
X_trn_tensor, Y_trn_tensor, M_trn_tensor = make_masked_tensors(X_trn_final, Y_trn_final)
X_val_tensor, Y_val_tensor, M_val_tensor = make_masked_tensors(X_val_final, Y_val_final)
...
optimizer = optimizer_(model.parameters(), lr=config.lr)
...
for epoch in range(config.num_epochs):
    ...
    for Xb, Yb, Mb in tqdm(loader_trn, desc=f"Epoch {epoch+1}/{config.num_epochs} train"):
        Xb = Xb.to(device)
        Yb = Yb.to(device)
        Mb = Mb.to(device)
    ...
    with torch.no_grad():
        for Xb, Yb, Mb in tqdm(loader_val, desc=f"Epoch {epoch+1}/{config.num_epochs} val"):
            Xb = Xb.to(device)
            Yb = Yb.to(device)
            Mb = Mb.to(device)
    ...
    print(
        ...
        f"train loss = {trn_loss:.5f} | val loss = {val_loss:.5f}"
    )
